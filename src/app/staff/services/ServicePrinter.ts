...
async createStorage(){
  ...
  initializeTicketVenteBefore(caisseDto: CaisseDTO){
    this.printVente='';
    const date: Date = new Date(caisseDto.caisseTransaction.datetransaction);
    this.printVente = '';
    ...
    this.printVente+='\n Bill: ' + caisseDto.caisseTransaction.commande.referencecommande;
    this.printVente+='\n Date: ' + date.getFullYear()+'/'+(date.getMonth()+1)+'/'+date.getDate()+'\t Time: '+date.getHours()+':'+date.getMinutes();
    this.printVente+='\n Vendeur  : ' + caisseDto.operateurnom==='' || caisseDto.operateurnom===null?this.authenService.utilisateur.username:caisseDto.operateurnom;
    this.printVente+='\n Mode de paiement : \t' + caisseDto.caisseTransaction.typepaiement;
    this.printVente+='\n -------------------------------------------';    this.printVente+='\n Designation \t Quantite \t Montant';
    caisseDto.caisseTransaction.commande.cartItems.forEach(c=>{
      if(c.quantite!==0){
        this.printVente+='\n '+ c.description.normalize('NFD').replace(/[\u0300-\u036f]/g, '').substr(0,15)+'\t'+c.quantite+'\t'+c.montant+'XAF';
      }
    });
    this.printVente+='\n -------------------------------------------';
    this.printVente+='\n Montant Total :\t\t' + caisseDto.caisseTransaction.commande.montantcommande+'XAF';
    this.printVente+='\n -------------------------------------------';
